...
import {
  ...
  IsKnownChainId,
  findExistingNetwork,
} from './ethereum-network-utils'
import { jsonRpcRequest } from './rpc.utils'
import { MESSAGE_TYPE } from './enums'

/**
  * Switches network if user approved request
  */
async function switchNetwork (updateRpcTarget, requestUserApproval, origin, rpcUrl,
  chainId, nickname, ticker) {
...
export async function addEthereumChain (req, res, end, addCustomRpc,
  sendMetrics, getCurrentChainId, findCustomRpcBy, updateRpcTarget,
  requestUserApproval) {
  ...
  const chainIdResult = await getValidChainId(chainId)
  if (typeof chainIdResult !== 'string') {
    return end(ethErrors.rpc.invalidParams(chainIdResult))
  }
  const _chainId = chainIdResult
  ...
  if (existingNetwork) {
    ...
    const result = await switchNetwork(updateRpcTarget, requestUserApproval,
      origin, existingNetwork.rpcUrl, existingNetwork.chainId,
      existingNetwork.nickname, existingNetwork.ticker)
    if (result) {
      return end(result)
    }
    res.result = null
  ...
  const result = await switchNetwork(updateRpcTarget, requestUserApproval,
    origin, firstValidRPCUrl, _chainId, _chainName, ticker)
  if (result) {
    return end(result)
  }
  res.result = null
...
export async function switchEthereumChain (req, res, end,
  getCurrentChainId, findCustomRpcBy, updateRpcTarget,
  requestUserApproval, setProviderType) {
  if (!req.params?.[0] || typeof req.params[0] !== 'object') {
    return end(
      ethErrors.rpc.invalidParams({
        message: `Expected single, object parameter. Received:\n${JSON.stringify(
