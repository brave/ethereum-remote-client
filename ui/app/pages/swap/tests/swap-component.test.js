...
describe('Swap Component', function () {
  ...
  const SwapTransactionScreen = proxyquire('../swap.component.js', {
    './swap.utils': utilsMethodStubs,
  }).default

  before(function () {
    sinon.spy(SwapTransactionScreen.prototype, 'componentDidMount')
    sinon.spy(SwapTransactionScreen.prototype, 'updateGas')
  })
  ...
  beforeEach(function () {
    wrapper = shallow((
      <SwapTransactionScreen
        ...
        updateSwapErrors={propsMethodSpies.updateSwapErrors}
        updateSwapTo={() => {}}
        updateSwapTokenBalance={propsMethodSpies.updateSwapTokenBalance}
  ...
  afterEach(function () {
    SwapTransactionScreen.prototype.componentDidMount.resetHistory()
    ...
    propsMethodSpies.updateSwapTokenBalance.resetHistory()
  ...
  it('should call componentDidMount', function () {
    assert(SwapTransactionScreen.prototype.componentDidMount.calledOnce)
  })
  ...
  describe('componentDidMount', function () {
    ...
    it('should call this.updateGas', async function () {
      ...
      wrapper.instance().componentDidMount()
      await timeout(250)
      assert.equal(SwapTransactionScreen.prototype.updateGas.callCount, 1)
    })
  })

  describe('componentWillUnmount', function () {
    it('should call this.props.resetSwapState', function () {
      propsMethodSpies.resetSwapState.resetHistory()
      assert.equal(propsMethodSpies.resetSwapState.callCount, 0)
      wrapper.instance().componentWillUnmount()
      assert.equal(propsMethodSpies.resetSwapState.callCount, 1)
    })
  })
  ...
  describe('componentDidUpdate', function () {
    ...
    it('should call updateSwapErrors with the expected params if swapToken is falsy', function () {
      ...
      assert.equal(propsMethodSpies.updateSwapErrors.callCount, 1)
      assert.deepEqual(
        propsMethodSpies.updateSwapErrors.getCall(0).args[0],
    ...
    it('should not call updateSwapTokenBalance or this.updateGas if network === prevNetwork', function () {
      SwapTransactionScreen.prototype.updateGas.resetHistory()
      propsMethodSpies.updateSwapTokenBalance.resetHistory()
      wrapper.instance().componentDidUpdate({
        from: {
    ...
    it('should not call updateSwapTokenBalance or this.updateGas if network === loading', function () {
    ...
    it('should call updateSwapTokenBalance and this.updateGas with the correct params', function () {
      SwapTransactionScreen.prototype.updateGas.resetHistory()
      propsMethodSpies.updateSwapTokenBalance.resetHistory()
      wrapper.instance().componentDidUpdate({
        from: {
      ...
      assert.equal(SwapTransactionScreen.prototype.updateGas.callCount, 1)
      assert.deepEqual(
        SwapTransactionScreen.prototype.updateGas.getCall(0).args,
        [],
      )
    })
  ...
  describe('render', function () {
    ...
    it('should pass the history prop to SwapHeader and SwapFooter', function () {
      ...
      assert.equal(wrapper.find(SwapHeader).length, 1)
      assert.equal(wrapper.find(SwapContent).length, 1)
      assert.equal(wrapper.find(SwapFooter).length, 1)
      assert.deepEqual(
    ...
    it('should pass showHexData to SwapContent', function () {
      wrapper.setProps({
        to: '0x80F061544cC398520615B5d3e7A3BedD70cd4510',
      })
      assert.equal(wrapper.find(SwapContent).props().showHexData, true)
    })
  })
  ...
  describe('validate when input change', function () {
    ...
    it('should warn when swap to a known token contract address', function () {
